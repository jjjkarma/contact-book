...
std::regex emailPattern("^[a-zA-Z0-9]+@[a-zA-Z0-9]+\\.[a-zA-Z]{2,}$"); // Email-адрес
std::regex spaces("(^\\s+)|(\\s+$)");  // Пробелы в начале и конце строки
std::regex numberPattern("^(\\+7|8)(\\(\\d{3}\\)\\d{7}|\\d{10}|\\(\\d{3}\\)-\\d{3}-\\d{2}-\\d{2})$");  // Номер телефона
std::regex datePattern("^(\\d{2})\\.(\\d{2})\\.(\\d{4})$");  // Дата в формате dd.mm.yyyy

Contact::Contact() {};  //constructor without params
